# 西煲 版本前瞻

> 本文件记录已定案但尚未实现的未来版本规划（供设计与决策溯源）。
> 当前已发布版本：v0.5.4
> 注：本文件（及 README/DEVELOPMENT.md）在 v0.6.0 英文界面适配时同步做中英双语。

## v0.5.5（规划中）

### 1. 标签树标签后显示内含数量

- 每个标签节点后显示当前标签含有的条目数
- 后端：聚合 `folder_tags` 按 tag_id 计数
- 前端：jsTree 渲染时标签名后加数量
- **口径待定**：直接数（本标签直接挂载）vs 含子级总数（递归合计）
  - 倾向含子级总数（与"点父级筛选含子级"的语义一致）
- 数量实时性：打标/清除后随 `loadTags` 刷新

### 2. 文件评分系统（完整设计）

**核心约束**
- 只能给**已打标签**的条目评分
- 评分逻辑建立在"标签筛选结果"之上（不允许纯评分筛选）
- 评分数据生命周期**完全跟随标签**：
  - 删除文件 → 清评分
  - 移动/重命名 → 受"标签迁移开关"控制（开=跟随，关=清理）
  - 随标签备份导出/导入
- 评分不进筛选方案（方案只管标签组合）

**打分（写入）**
- 入口：快捷键 `W` + 右键菜单「评分…」
- 浮窗：五颗星槽，可点左/右半，0.5 粒度（5 星=10 档），最低半星
- 浮窗含「清除评分」按钮
- 多选时：批量给所有选中条目设同一分数
- 只能对有标签条目弹出（无标签条目无评分入口）

**显示**
- 条目左上角常显角标 `3.5⭐`
- 工具栏显示评分开关 + 快捷键 `TAB` 切换角标显示/隐藏（默认显示）

**筛选（读取）**
- 入口：⭐ 开关，位置在筛选模式 ➕ 按钮与 📚 方案按钮之间
- 仅在 🔖 标签筛选模式激活后可用（评分筛选依赖标签筛选）
- 点击展开面板（停靠在筛选区，不遮挡文件列表）：
  - 星槽选星级（0.5 粒度）
  - 文件 / 文件夹 两个复选框（可只文件、只文件夹、都勾）
- 设置后收起，筛选胶囊条追加状态显示 `★≥4 · 仅文件`
- 逻辑：先标签筛选 → 再评分叠加 → 再类型过滤
- 星级为 ≥ 阈值（选 4 星 = 4 星及以上）
- 未评分条目默认 0 分，不满足 ≥ 阈值时被过滤

**排序**
- 列表排序菜单新增「评分」维度（按星级降序）

**存储**
- 新增 `path_ratings(path, rating)` 表（平行于 folder_tags / path_aliases）
- 跟随标签的删除/移动/备份逻辑

## 未来计划

### 标签名搜索

- 标签树顶部加搜索框，输入子串实时高亮/过滤匹配的标签节点
- 优先级高于"标签别名"（子串模糊匹配覆盖大部分别名需求）

## v0.6.0（重构，进行中）

> 重构原则：**底层逻辑优先**——先立地基与接口，再谈功能；不先做会让后续功能难以接入的项，优先做。
> **进度**：✅ 第 1 步完成（模块解耦，git commit 已留痕）

按依赖顺序（先做的为后做的打地基）：

### 第一梯队：地基（必须最先，后续全依赖）
1. ✅ **模块解耦**（已完成）：app.py 按域拆分 Blueprint（routes/tags, files, search, settings），API 路径不变，前端零改动，57 单测全过
2. ⬜ **数据库容错**（下一步）：启动完整性校验（PRAGMA integrity_check）、损坏自动修复、定时快照可回滚——数据模型将大改，容错必须先立
3. ⬜ **稳定文件标识**：NTFS 文件 ID / volume+file index 作标签锚点，路径只是当前地址——根治"文件移动/重命名后标签丢失"，数据锚点变更影响所有路径级功能（标签/备注名/剪贴板）

### 第二梯队：架构接口（随解耦定义，供未来功能接入）
4. **右键外部工具集成层**：启动检测已装软件（7-Zip/WinRAR/Locale Emulator 等），右键菜单动态加入对应动作（调其 CLI）——用户"脱离原生资源管理器"的关键，需在模块化时定义接口
5. **缩略图可插拔后端**：优先系统 COM 缩略图（覆盖广、与原生一致），失败回退 PyAV——当前 PyAV 是妥协，重构后双后端覆盖 PSD/3D 等格式
6. **导航源抽象**：盘符 / Known Folders / 标签视图统一为导航接口——为虚拟导航预留

### 第三梯队：重构收益项
7. **层级展开重构**：不物化祖先链，查询期 BFS 展开 + flat_dict 缓存（借鉴 etiquette）——治本"属性父级冗余/叶子升父级幽灵标签"
8. **开放外部标签写入 API**：`POST /api/external/tags`，定义对外标准，供插件/第三方接入
9. **标签筛选返回交互**：点标签进历史，返回回目录；多标签组合的进史策略
10. **英文界面适配**：i18n 界面文案 + 帮助文档 + 更新日志，设置加语言选项（中文/English）
11. **增量更新**（含检查更新）：文件清单/哈希、独立更新器、失败回滚，配合重构规划发布体系

### v0.6.0 之后
- **剪贴板批量操作**：对齐 Win11 资源管理器（Ctrl+X 剪切 / Ctrl+C 复制 / Ctrl+V 移动或复制，路径清单式；源文件剪切后半透明状态）
- **多标签页**：对齐 Win11 标签页，每标签页独立目录/筛选/历史/选中状态，切换不丢——依赖前端状态重构
- **虚拟导航落地**：文件树接入 Known Folders（桌面/下载/图片/视频等）
- **预览窗格**：可选，持续对比场景（优先级低）

## 远期

- **可视化统计**：标签使用量排行 / 标签树热力 / 标签共现分析（后端聚合 API + 前端图表），等 v0.6.0 数据模型稳定后做
- **视觉识别插件**（BYOK）：独立分发，内部调 Ollama/云端 API 识别，经外部 API 写入西煲；先极简 CLI 验证架构，再升级 GUI
  - 参考：baessu/eagle-auto-tagger（标签收敛）、Duelion/homebox-companion（BYOK/限流）
- **虚拟导航**：文件树接入系统 Known Folders（桌面/下载/图片/视频等），重构时将导航源抽象为接口（盘符/Known Folders/标签视图），后续扩展易

## 决策记录（内部）

功能取舍的详细决策见内部开发文档 `DEVELOPMENT.md`，不在此公开维护。
