...
- **复制标签树**（v0.6.1 候选，设计已定案，2026-08 讨论）：
  ...
  - **挂载点选择**：弹窗内 jsTree 单选挂载点；也可挂根级
  - **同名冲突**：生成的标签名与目标父级下已有同名标签 → 复用已有标签（避免重复建）
  - **依赖前置**：原记录说需解决"父级标签显示/管理"——已由 v0.6.0 标签异常区解决；自动打标用 `set_folder_tags` 递归（程序批量不拦父级）
  - **技术**：后端 `os.walk` 递归目录（复用平铺文件夹的遍历调研）+ `add_tag`/`get_or_create_tag` 建层级 + `set_folder_tags` 打标；批量写入需防大目录卡顿
  - **工作量**：后端 0.5-1 天 + 前端（右键菜单+挂载点弹窗）0.5-1 天
