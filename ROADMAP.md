...
### 第一梯队：地基（必须最先，后续全依赖）
1. **模块解耦**：app.py 按域拆分（api_tags/api_files/api_search/api_settings），前端按域聚合——一切重构的地基，不先拆则后续改动全叠在上帝模块上
2. **数据库容错**：启动完整性校验（PRAGMA integrity_check）、损坏自动修复、定时快照可回滚——数据模型将大改，容错必须先立
3. **稳定文件标识**：NTFS 文件 ID / volume+file index 作标签锚点，路径只是当前地址——根治"文件移动/重命名后标签丢失"，数据锚点变更影响所有路径级功能（标签/备注名/评分/剪贴板）
