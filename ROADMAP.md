...
> **进度**：✅ 第 1、2、4、5、6、8 步完成（模块解耦、数据库容错、右键菜单+外部工具、缩略图后端、导航源抽象、外部写入API）；⬜ 第 3 步暂缓（先做 4-11 见效快项）
...
8. ✅ **开放外部标签写入 API**（已完成）：`POST /api/v1/tags/apply`（批量打标，支持路径/标签名/父标签/来源）+ **安全区路径白名单**（meta `api_allow_roots`，范围外拒绝）+ **审核队列**（默认进 `pending_tag_applies` 表，前端「🕓 待审核」独立待办入口批量接受/拒绝）；PhotoPrism FirstOrCreate 幂等语义；87 单测
