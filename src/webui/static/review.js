...
function renderTaskGrid() {
  ...
  for (const g of pendingGroups) {
    ...
    card.innerHTML = `
      ...
      <div class="review-card-meta muted">${isReview ? '来源: ' + escHtml(srcs) : '孤儿挂载（父级标签无子级伴生）'}</div>
...
async function clearOrphans() {
  if (!confirm('确定清理所有孤儿挂载吗？\n\n将移除这些文件上的父级标签关联（不影响挂子级文件的标签）。')) return;
  try {
